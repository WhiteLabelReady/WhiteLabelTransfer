...
class WeTransfer_Users
{
		...
		*/
		...
		while ( $aRow = mysql_fetch_array( $rResult ) ) {
			...
			for ( $i = 0; $i < count( $aColumns ); $i++ ) {
				switch( $aColumns[$i] ) {
					case 'site_language':
						$row[] = $this->_WeTransfer_Languages->fetchFriendlyNameById( $aRow[ $aColumns[$i] ] );
						break;
    ...
    */
    public function login( $username, $password )
    {
        ...
        if( mysql_num_rows( $res ) > 0 ) {
            $data					= mysql_fetch_assoc( $res );
            $data['media_count']	= $this->_WeTransfer_User_Media->fetchMediaCountByUserId( $data['id'] );            
    ...
    {
    	$sql    = "SELECT `email` FROM `".DB_TABLE_PREFIX."users` ";
    	$sql   .= "WHERE `id` = '".mysql_real_escape_string( $userId )."' ";
    	$sql   .= "LIMIT 1 ";
