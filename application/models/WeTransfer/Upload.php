...
class WeTransfer_Upload
{
	...
	public function zipFilesViaBash( $token, $destination, $files = array(), $sourcePath )
	{	
		if( empty( $files ) ) {
			return false;
		}
		
		$endpoint	= $destination.'/'.SITE_ARCHIVE_PREFIX.$token.'.zip';
		$log		= BASEDIR.'/data/logs/zip/'.$token.'.log';
		$errorLog	= BASEDIR.'/data/logs/errors/'.$token.'.log';
		$filesToZip	= implode( ' ', $files );		
		$cmd		= 'cd '.dirname( $sourcePath ).' && zip -vjrm '.$endpoint.' '.basename( $sourcePath ).' > '.$log.' 2>&1';
		$result		= '';
		// zip the files
		$zip		= exec( $cmd, $output, $result );		
		
		file_put_contents( BASEDIR.'/data/logs/zipOutput.log', $zip );
		file_put_contents( BASEDIR.'/data/logs/zipResult.log', $result );
    ...
    */
    ...
    {    		    	
        ...
        $this->_WeTransfer_Users->updateUserById( $data['uploader'], array('last_upload' => time() ) );
    ...
    public function addUploadedFiles( $parentId, $data = array() )
    {
    	...
    	foreach( $data AS $key => $value ) {
			...
			$sql .= "'".mysql_real_escape_string( $value['path'] )."', ";
			$sql .= "'".mysql_real_escape_string( $value['size'] )."' ";			
			$sql .= "); ";
